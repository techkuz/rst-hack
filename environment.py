...
class Environment:
    ...
    def main_logic(self) -> None:
        ...
        for minute in range(self.START_MINUTES, self.END_MINUTES+1, 1):
            for courier_id in self.couriers:
                if not self.couriers[courier_id].order_info:
                    continue
                courier_action = self.couriers[courier_id].order_info[1]
                courier_order = self.couriers[courier_id].order_info[0]

                self.couriers[courier_id].time +=1

                if courier_action == 'pickoff':
                    if courier_order.pickup_from < minute < courier_order.pickup_to:
                        action = courier_id.order_info.pop()
                        courier_id.has_order = True
                    ...
                    if courier_order.dropoff_from < minute < courier_order.dropoff_to:
                        ...
                        self.orders[courier_id.order_info[0]].delivered = True
                        action = courier_id.order_info.pop()
                        courier_id.has_order = False

                        if not courier_id.order_info:
                            courier_id.is_active = False
        ...
        return self.couriers, self.orders, profit
